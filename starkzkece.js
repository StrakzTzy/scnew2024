...
const { ngazap } = require('./all/bugStarkz/ngazap')
const { buttonkal } = require('./all/bugStarkz/buttonkal')
const { cttl } = require('./all/bugstarkz/cttl')
const { tizi } = require('./all/bugstarkz/tizi')
const { weg } = require('./all/bugstarkz/weg')
const { virtex7 } = require('./all/bugstarkz/virtex7')
...
"title": "SC BY Starkz-dev`", 
"containsAutoReply": true, 
"mediaType": 1, 
"thumbnail": m, 
"mediaUrl": "https://www.instagram.com/zal_x_u", 
"sourceUrl": "https://www.instagram.com/zal_x_u" }}}, { quoted: m }) }
...
*ᕤ•˙ ⃞✦ SCRIPT  : Starkz-dev*
*ᕤ•˙ ⃞✦ BOT NAME : Starkz-dev BOT*
*ᕤ•˙ ⃞✦ PENGGUNA NAMA : ${namabot}*
*ᕤ•˙ ⃞✦ BUY SC : 6285771598591*
*ᕤ•˙ ⃞✦ BUY BOT : 6285771598591*
...
🇸‌🇴‌🇸‌🇲‌🇪‌🇩‌
> https://www.instagram.com/zal_x_u
SUBSCRIBE BIAR GANTENG🥰
> https://www.youtube.com/@zal_x_u
`
haikal.sendMessage(from, { image: { url: `https://telegra.ph/file/4d5ac8872b727934e431d.jpg` }, caption: text12, fileLength: 10, contextInfo: { mentionedJid:[sender], forwardingScore: 9999, isForwarded: true }}, { quoted: m })
}
break
case 'payment':{
const owned = `${global.ownerNumber}@s.whatsapp.net`
let rsm = `*-------「 PAYMENT BY Starkz-dev」 -------*
...
> *KAMI PROSES*`
haikal.sendMessage(from, { image: { url: `https://telegra.ph/file/4d5ac8872b727934e431d.jpg` }, caption: `${rsm}` }, { quoted: m })
}
break
case "tts": case "gtts":{
...
case "sc" :
case "script": {
const text12 = `> UNTUK SCRIPT BISA LANGSUNG TANYA KE Starkz-dev AJA KAK INI NOMORNYA 085926340758 BTW DIA JUALAN PANEL JUGA`
reply(text12)
}
break
case "yt": {
...
let barang = t[0];
let nominal = t[1];
reply(`> *TRANSAKSI BERHASIL Starkz-DEV*
▬▭▬▭▬▭▬▭▬▬▭▬▭
...
*NOMINAL* : Rp${nominal}
*SISTEM :* DIRECT
> *SOSMED Starkz-dev :* https://www.instagram.com/starkzstur
▬▭▬▭▬▭▬▭▬▬▭▬▭
> *NOTE:* TERIMA KASIH TELAH ORDER DAN MEMPERCAYAI *Starkz-dev* JANGAN LUPA ORDER LAGI YA🙏`)
}
        break
case 'proses':{
m.reply('*SIAP PESANAN ANDA AKAN KAMI PROSES JADI DI MOHON UNTUK MENUNGGU SEBENTAR YAH BANG*')
haikal.sendMessage(owner + "@s.whatsapp.net", { text: "BANG Starkz-dev ADA YANG MESEN NIH CEPETAN PROSES NANTI BUYER NGAMOK", contextInfo:{ forwardingScore: 9999, isForwarded: true }})
}
break
case "formatneed": {
...
🥀BEE SMART BUYER🥀`
haikal.sendMessage(from, { image: { url: `https://telegra.ph/file/3e86f2c834ac0490a1231.jpg` }, caption: text12, fileLength: 10, contextInfo: { mentionedJid:[sender], forwardingScore: 9999, isForwarded: true }}, { quoted: m })
}
break
case "feerekber": {
...
break
case "listpanel": {
const text12 = `*LIST HARGA PANEL RUN BOT BY Starkz-dev⚡*
...
break
case "produklain": {
const text12 = `> *OPEN ADMIN PANEL 30K*
*RESELLER PANEL 25K*
...
case "addprem":{
if (!isOwner) return reply(mess.only.owner)
if (!args[0]) return reply(`Penggunaan ${prefix+command} nomor\nContoh ${prefix+command} 6285771598591`)
...
let groups = Object.entries(getGroups).slice(0).map((entry) => entry[1])
let anu = groups.map((v) => v.id)
let teks = `⬣ *LIST GROUP BY Starkz-dev*\n\n`
for (let x of anu) {
let metadata2 = await haikal.groupMetadata(x)
...
case "listgroup":{
if (!isOwner) return (`> Ngapain ? Khusus Starkz-dev Aja !!`)
let getGroups = await haikal.groupFetchAllParticipating()
let groups = Object.entries(getGroups).slice(0).map((entry) => entry[1])
let anu = groups.map((v) => v.id)
let hituet = 0
...
break
case "jpm": case "jpm2":{
if (!isOwner) return reply(`> Khusus Bang Starkz-dev`)
...
reply("*SUCCESFUL BANG Starkz-dev✅*")
}
break
case "pushkontakid":{
if (!isOwner) return reply(`> Khusus Bang Starkz-dev`)
...
"BEGIN:VCARD",
"VERSION:3.0",
`FN:starkz[${createSerial(1)}] ${contact.split("@")[0]}`,
...
break
case "pushkontakgc":{
if (!isOwner) return reply(`> Khusus Starkz-dev`)
...
Penggunaan:
${prefix + command} |nama pake garus lurus sebelum nama yah bang Starkz-dev | yang kaya gitu`);
let nama = t[0];
let nominal = t[1];
        reply(mess.save)
reply(`> system otomatis save done ✅

> kontak : ${nominal}

> jangan lupa save back  ${namabot} ya`)
}
        break        
case "pushkontakidjd":
if (!isOwner) return reply(`> Khusus Starkz-dev`)
if (!text) return reply(`> Penggunaan Salah Silahkan Gunakan Command Seperti Ini\n${prefix+command} idgroup|jeda|teks\nUntuk Liat Id Group Silahkan Ketik .idgroup`)
await reply("> Laksanakan Starkz-dev")
...
reply("> Succes Starkz-dev!")
break
case "pushkontakgcjd":
if (!isOwner) return reply(`> Khusus Starkz-dev`)
if (!isGroup) return reply(mess.only.group)
if (!text) return reply(`> Penggunaan Salah Silahkan Gunakan Command Seperti Ini\n${prefix+command} jeda|teks`)
await reply("> Laksanakan Bang Starkz-dev")
...
"BEGIN:VCARD",
"VERSION:3.0",
`FN:starkz[${createSerial(2)}] ${contact.split("@")[0]}`,
`TEL;type=CELL;type=VOICE;waid=${contact.split("@")[0]}:+${contact.split("@")[0]}`,
"END:VCARD",
"", ].join("\n");
return vcard; }).join("");
...
if (!isGroup) return m.reply(`> Khusus Group Kontol`)
huhuhs = await haikal.sendMessage(m.chat, {
    text: `> *ID :* ${groupMetadata.id}\n\nJangan Lupa Di Salin Idnya Yah Bang Starkz-dev*`
}, {quoted: m, ephemeralExpiration: 86400})
await sleep(1000) // (?); mengirim kontak seluruh member
reply
break
case 'savekontak': case 'svkontak':
if (!isOwner) return reply(`> Khusus Starkz-dev Aja`)
if (!isGroup) return m.reply(`> Khusus Group Kontol`)
